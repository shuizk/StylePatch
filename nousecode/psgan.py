...
parser.add_argument('--max_count', type=int, default=3000, help='max number of iterations to find adversarial example') # 寻找对抗样本最大迭代次数
...
parser.add_argument('--manualSeed', type=int, default=5963, help='manual seed') # 手工种子
...
def train(epoch):
    ...
    for batch_idx, (_, (data,labels)) in zip(training_idx, enumerate(train_loader)):
        ...
        patch, patch_shape = init_patch_square(image_size, patch_size, patch_file="full_numpy_bitmap_cruise ship.npy") 
        # else:
        ...
        adv_x, mask, patch = attack(data, labels, origin_patch, style_img, patch, mask)
...
def attack(x, labels, content_img, style_img, patch, mask):
    # 将局部图像插值扩大到 整体大小
    # style_img = F.interpolate(style_img, size=(image_size,image_size), mode='bicubic')
    # content_img = copy.deepcopy(x)
    model, PS_losses, style_losses = get_model_and_PSlosses(netClassifier, content_img, style_img, default_content_layers, default_style_layers, opt.netClassifier)
