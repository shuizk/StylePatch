...
def basic_visualize(input_, gradients, save_path=None, weight=None, cmap='jet', alpha=0.7):
...
def my_visualize(nets, input_, gradients, confidences, weighted_cam_map, save_path=None, weight=None, cmap='jet', alpha=0.7):
    """ Method to plot the explanation.
    ...
    input_ = format_for_plotting(denormalize(input_))
    subplots = [
        ('input_image', [(input_, None, None)])
    ]
    shape = weighted_cam_map.shape[0]
    for gradient, i in zip(gradients,range(len(gradients))):
        gradients[i] = format_for_plotting(gradient)
        if gradients[i].shape[0] != shape:
            if gradients[i].shape[0] == 299:
                resize = scipy.ndimage.zoom(gradients[i].numpy(),0.7491)
            else:
                resize = scipy.ndimage.zoom(gradients[i].numpy(),1.3348)
            gradients[i] = torch.tensor(resize)
        subplots.append((nets[i][0]+" "+str(confidences[i]), [(input_, None, None), (gradients[i], cmap, alpha)]))
    
    subplots.append(("weighted_cam_map", [(input_, None, None), (weighted_cam_map, cmap, alpha)]))
    num_subplots = len(subplots)

    fig = plt.figure(figsize=(num_subplots * 8, 8))
...
def find_inception_v3_layer(arch, target_layer_name):
    target_layer = arch._modules[target_layer_name]
    return target_layer
